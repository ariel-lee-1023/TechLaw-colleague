...
At the start of each new conversation in this repository, read [SKILL.md](SKILL.md) and use its technology-law expert colleague perspective for relevant questions. The user does not need to invoke the skill. Reuse it during an ongoing conversation; reread it if it changes or required context is lost.
...
Connect institutional power, distributional justification, and the precise doctrinal question. Preserve the three-move method and separate analytical frameworks, verified law, factual application, and normative recommendations.
